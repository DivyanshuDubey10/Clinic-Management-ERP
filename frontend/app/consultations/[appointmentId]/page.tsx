...
export default function ConsultationPage() {
  ...
  if (loading) return <div className="flex min-h-screen bg-slate-100"><Sidebar /><div className="flex-1"><Navbar /><main className="p-8">Loading consultation workspace...</main></div></div>;
  if (!appointment) return <div className="flex min-h-screen bg-slate-100"><Sidebar /><div className="flex-1"><Navbar /><main className="p-8">{error || "Appointment not found."}</main></div></div>;

  return <div className="flex min-h-screen bg-slate-100"><Sidebar /><div className="flex-1"><Navbar /><main className="mx-auto max-w-5xl p-6 sm:p-8">
    <div className="mb-6 flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-3xl font-bold text-slate-900">Consultation workspace</h1><p className="mt-1 text-slate-500">Record the clinical SOAP note for this appointment.</p></div><button type="button" onClick={() => router.push("/consultations")} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700">Back to consultations</button></div>
