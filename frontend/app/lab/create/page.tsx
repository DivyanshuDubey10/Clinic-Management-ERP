...
export default function CreateLabOrderPage() {
  ...
  return <div className="flex min-h-screen bg-muted/50"><Sidebar /><div className="min-w-0 flex-1"><Navbar /><main className="mx-auto max-w-3xl p-5 sm:p-8"><h1 className="text-3xl font-bold tracking-tight text-foreground">Create Lab Order</h1><p className="mt-2 text-muted-foreground">Order laboratory investigations for a patient consultation.</p>
    <form onSubmit={handleSubmit} className="mt-8 space-y-6 rounded-2xl border border-border bg-card p-6 shadow-sm sm:p-8">
      ...
      <Field label="Test name"><input required name="testName" value={formData.testName} onChange={handleChange} placeholder="CBC, X-Ray Chest, MRI Brain..." className="w-full rounded-xl border border-border px-3.5 py-3 text-sm outline-none transition placeholder:text-muted-foreground focus:border-cyan-400 focus:ring-4 focus:ring-cyan-100" /></Field>
      ...
      <Field label="Notes (optional)"><textarea rows={4} name="notes" value={formData.notes} onChange={handleChange} placeholder="Additional instructions..." className="w-full resize-y rounded-xl border border-border px-3.5 py-3 text-sm outline-none transition placeholder:text-muted-foreground focus:border-cyan-400 focus:ring-4 focus:ring-cyan-100" /></Field>
...
function Select({ label, children, ...props }: { label: string; children: ReactNode } & SelectHTMLAttributes<HTMLSelectElement>) { return <Field label={label}><select {...props} className="w-full rounded-xl border border-border px-3.5 py-3 text-sm outline-none transition focus:border-cyan-400 focus:ring-4 focus:ring-cyan-100 disabled:cursor-not-allowed disabled:bg-muted/50">{children}</select></Field>; }
