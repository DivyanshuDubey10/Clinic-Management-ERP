...
//   return <label className="block"><span className="text-sm font-medium text-slate-700">{label}</span><input type={type} name={name} value={value} onChange={onChange} disabled={disabled} className={`mt-2 h-11 w-full rounded-xl border px-3.5 text-sm outline-none transition ${disabled ? "cursor-not-allowed border-slate-100 bg-muted/50 text-muted-foreground" : "border-border bg-card text-card-foreground hover:border-border focus:border-cyan-400 focus:ring-4 focus:ring-cyan-100"}`} /></label>;
// }
...
function Input({ label, name, value, onChange, disabled, type = "text", maxLength, inputMode, pattern }: { label: string; name: keyof ProfileForm; value: string; onChange?: (event: ChangeEvent<HTMLInputElement>) => void; disabled: boolean; type?: string; maxLength?: number; inputMode?: "numeric" | "text" | "tel" | "email"; pattern?: string }) {
  return <label className="block"><span className="text-sm font-medium text-slate-700">{label}</span><input type={type} name={name} value={value} onChange={onChange} disabled={disabled} maxLength={maxLength} inputMode={inputMode} pattern={pattern} className={`mt-2 h-11 w-full rounded-xl border px-3.5 text-sm outline-none transition ${disabled ? "cursor-not-allowed border-slate-100 bg-muted/50 text-muted-foreground" : "border-border bg-card text-card-foreground hover:border-border focus:border-cyan-400 focus:ring-4 focus:ring-cyan-100"}`} /></label>;
}
